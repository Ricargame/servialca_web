...
export const ModalZona = (props) => {
  ...
  const actualizarCertificado = async () => {
    ...
    if (operacion === 1) {
      endpoint = op.conexion + "/zona/registrar";
    } else if (operacion === 2) {
      endpoint = op.conexion + "/zona/actualizar";
      bodyF.append("ID", values.zona_id);
    } else {
      endpoint = op.conexion + "/zona/eliminar";
      bodyF.append("ID", values.zona_id);
    }
  ...
  const selecionarzona = async (id) => {
    let endpoint = op.conexion + "/zona/ConsultarUno?ID=" + id;
    ...
    await fetch(endpoint, {
      ...
      .then((response) => {
        setActivate(false);
        console.log(response);
        txtDescripcion.current.value = response.zona_nombre
        setValues(response);
      })
      .catch((error) =>
        setMensaje({
